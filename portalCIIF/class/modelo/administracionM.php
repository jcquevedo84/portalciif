<?php
	/**
	*CIIF administracionM
	*
	*Clase Modelo de la Pagina administracion.php
	*En esta clase se encuentran todos los modelos de datos y conexiones a base de datos realizadas desde la pagina info.php
	*@copyright CANTV
	*@author Kery Pérez
	*@version 1.0
	*@package administracionM
	*
	*/
	class administracionM extends templeteM 
	{
		/**
		*
		*Constructor
		...
		*/
		...
		{	
	    	...
	    	$resulQuery=$objPG->insercion("0,".$form["txtnombre"].";1,".$form["txtdescripcion"].";2,".$user->getIdUsuario().";3,".$ip.";",__LINE__);
			unset($objPG);
			$rutaLogs=varConf::getLogs();
			$mensaje= new mensaje(1,$rutaLogs["administracion"],"El usuario con id: ".$user->getIdUsuario()." y nombre: ".$user->getname." registro un rol desde la ip: ".$ip,"administracion",__CLASS__,__FUNCTION__,__LINE__);
		...
		*/
		...
		{		   
			...
			$rutaLogs=varConf::getLogs();
			$user=$this->objS->get("user");
			$mensaje= new mensaje(1,$rutaLogs["administracion"],"El usuario con id: ".$user->getIdUsuario()." y nombre: ".$user->getname." Actualizo un usuario desde la ip: ".$ip,"administracion",__CLASS__,__FUNCTION__,__LINE__);
		...
		*/
		function actdesRol($id="",$est="")
		{
			...
			$user=$this->objS->get("user");
			if($est)
				$mensaje= new mensaje(1,$rutaLogs["administracion"],"El usuario con id: ".$user->getIdUsuario()." y nombre: ".$user->getname." Activo un usuario desde la ip: ".$ip,"administracion",__CLASS__,__FUNCTION__,__LINE__);
			else
				$mensaje= new mensaje(1,$rutaLogs["administracion"],"El usuario con id: ".$user->getIdUsuario()." y nombre: ".$user->getname." Desactivo un usuario desde la ip: ".$ip,"administracion",__CLASS__,__FUNCTION__,__LINE__);
